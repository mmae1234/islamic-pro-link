...
DROP FUNCTION IF EXISTS public.is_business_team_member(uuid, uuid);
DROP FUNCTION IF EXISTS public.has_business_role(uuid, uuid, business_member_role[]);
...
-- Now recreate all business_accounts policies using the safe functions
DROP POLICY IF EXISTS "Business team members can view business accounts" ON public.business_accounts;
DROP POLICY IF EXISTS "Contact info for business relationships" ON public.business_accounts;
...
-- Recreate policies with safe functions
